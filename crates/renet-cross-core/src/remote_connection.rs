...
pub struct RenetClient {
    ...
    // Receipt history must be retained until confirmed, but only received data
    // requests an immediate ACK. An ACK-only packet must not elicit another ACK.
    ack_pending: bool,
...
impl RenetClient {
    ...
    /// Returns the round-trip time sampled from acknowledged data packets.
    /// Zero until the first sample; receive-only traffic does not refresh it.
    pub fn rtt(&self) -> f64 {
        self.rtt
    }

    /// Returns the unacknowledged fraction of sent data packets in the statistics window.
    /// ACK-only packets are excluded: they do not require an immediate reply.
    ...
    pub fn process_packet(&mut self, packet: &[u8]) {
        ...
        self.ack_pending |= !matches!(packet, Packet::Ack { .. });
        ...
        match packet {
            ...
            Packet::Ack { ack_ranges, .. } => {
                ...
                for packet_sequence in new_acks {
                    ...
                    // ACK-only receipts can be deferred until the peer next has
                    // data to send. They retire receipt history, but cannot provide
                    // meaningful data-loss or RTT samples.
                    if let PacketSentInfo::Ack { largest_acked_packet } = sent_packet.info {
    ...
    pub fn get_packets_to_send(&mut self) -> Vec<Payload> {
        ...
        let data_packets = packets.len() as u64;
        // Respond on the very next flush to data (including duplicates/retries).
        // Otherwise, include retained receipts only alongside outgoing data.
        // Repeated flushes with no new work must return no packets.
        if !self.pending_acks.is_empty() && (self.ack_pending || !packets.is_empty()) {
        ...
        self.stats.sent_packets(data_packets, bytes_sent);
...
mod tests {
    ...
    fn deliver(packets: Vec<Payload>, receiver: &mut RenetClient) {
        for packet in packets {
    ...
    #[test]
    fn a_single_message_is_acked_immediately_and_then_both_peers_become_idle() {
        for channel in [DefaultChannel::Unreliable, DefaultChannel::ReliableOrdered] {
            ...
            sender.send_message(channel, Bytes::from_static(b"one message"));
            deliver(sender.get_packets_to_send(), &mut receiver);
            assert_eq!(receiver.receive_message(channel).unwrap(), b"one message"[..]);

            // No timer advancement, application data, or snapshot is necessary to ACK.
            let acks = receiver.get_packets_to_send();
            assert_eq!(acks.len(), 1);
            assert!(matches!(
                Packet::from_bytes(&mut octets::Octets::with_slice(&acks[0])).unwrap(),
                Packet::Ack { .. }
            ));
            assert!(receiver.get_packets_to_send().is_empty());
            deliver(acks, &mut sender);

            for _ in 0..30_000 {
                sender.update(Duration::from_millis(1));
                receiver.update(Duration::from_millis(1));
                assert!(sender.get_packets_to_send().is_empty());
                assert!(receiver.get_packets_to_send().is_empty());
            }
            assert!(sender.sent_packets.is_empty());
            assert!(receiver.sent_packets.is_empty());
        }
    }

    #[test]
    fn a_lost_ack_is_recovered_by_the_existing_reliable_retry() {
        let mut sender = RenetClient::new(ConnectionConfig::default());
        let mut receiver = RenetClient::new(ConnectionConfig::default());
        sender.send_message(DefaultChannel::ReliableOrdered, Bytes::from_static(b"reliable"));
        deliver(sender.get_packets_to_send(), &mut receiver);
        assert!(receiver.receive_message(DefaultChannel::ReliableOrdered).is_some());
        assert_eq!(receiver.get_packets_to_send().len(), 1); // Drop this ACK.
        assert!(receiver.get_packets_to_send().is_empty());

        sender.update(Duration::from_secs(1));
        receiver.update(Duration::from_secs(1));
        let retry = sender.get_packets_to_send();
        assert!(!retry.is_empty());
        deliver(retry, &mut receiver);
        assert!(receiver.receive_message(DefaultChannel::ReliableOrdered).is_none());
        let acks = receiver.get_packets_to_send();
        assert_eq!(acks.len(), 1, "duplicate reliable data must elicit another ACK");
        deliver(acks, &mut sender);
        sender.update(Duration::from_secs(1));
        assert!(sender.get_packets_to_send().is_empty());
    ...
    #[test]
    fn data_receipts_are_sent_without_delay_and_control_receipts_do_not_distort_stats() {
        let mut a = RenetClient::new(ConnectionConfig::default());
        let mut b = RenetClient::new(ConnectionConfig::default());
        a.send_message(DefaultChannel::Unreliable, Bytes::from_static(b"first"));
        deliver(a.get_packets_to_send(), &mut b);
        a.update(Duration::from_millis(50));
        b.update(Duration::from_millis(50));
        deliver(b.get_packets_to_send(), &mut a);
        assert_eq!(a.rtt(), 0.05);
        assert!(a.get_packets_to_send().is_empty());

        // B's ACK can remain unconfirmed until A has data to send. This is not
        // packet loss and its eventual confirmation is not a two-second RTT.
        a.update(Duration::from_secs(2));
        b.update(Duration::from_secs(2));
        assert_eq!(b.packet_loss(), 0.0);
        a.send_message(DefaultChannel::Unreliable, Bytes::from_static(b"second"));
        let packets = a.get_packets_to_send();
        assert_eq!(packets.len(), 2, "outgoing data carries a companion receipt for B's ACK");
        deliver(packets, &mut b);
        assert_eq!(b.rtt(), 0.0);
        assert!(b.sent_packets.is_empty(), "confirmed control packet bookkeeping is retired");
        assert_eq!(b.get_packets_to_send().len(), 1);
        assert!(b.get_packets_to_send().is_empty());
        assert!(b.bytes_sent_per_sec() > 0.0, "byte statistics still include control traffic");
    }

    #[test]
    fn loss_statistics_still_count_unacknowledged_data() {
        let mut a = RenetClient::new(ConnectionConfig::default());
        let mut b = RenetClient::new(ConnectionConfig::default());
        a.send_message(DefaultChannel::Unreliable, Bytes::from_static(b"lost"));
        assert_eq!(a.get_packets_to_send().len(), 1); // Drop the first data packet.
        a.send_message(DefaultChannel::Unreliable, Bytes::from_static(b"received"));
        deliver(a.get_packets_to_send(), &mut b);
        deliver(b.get_packets_to_send(), &mut a);
        for _ in 0..1_000 {
            a.update(Duration::from_millis(1));
            b.update(Duration::from_millis(1));
        }
        assert_eq!(a.packet_loss(), 0.5);
        assert_eq!(b.packet_loss(), 0.0);
    }

    #[test]
    fn reordered_and_duplicate_data_are_acked_and_reliable_messages_deliver_once_in_order() {
        let mut a = RenetClient::new(ConnectionConfig::default());
        let mut b = RenetClient::new(ConnectionConfig::default());
        a.send_message(DefaultChannel::ReliableOrdered, Bytes::from_static(b"first"));
        let first = a.get_packets_to_send();
        a.send_message(DefaultChannel::ReliableOrdered, Bytes::from_static(b"second"));
        let second = a.get_packets_to_send();
        deliver(second, &mut b);
        assert!(b.receive_message(DefaultChannel::ReliableOrdered).is_none());
        deliver(b.get_packets_to_send(), &mut a);
        deliver(first.clone(), &mut b);
        assert_eq!(b.receive_message(DefaultChannel::ReliableOrdered).unwrap(), b"first"[..]);
        assert_eq!(b.receive_message(DefaultChannel::ReliableOrdered).unwrap(), b"second"[..]);
        deliver(b.get_packets_to_send(), &mut a);
        deliver(first, &mut b);
        assert!(b.receive_message(DefaultChannel::ReliableOrdered).is_none());
        assert_eq!(b.get_packets_to_send().len(), 1);
    ...
    #[test]
    fn one_way_traffic_retires_receipt_history_without_an_ack_feedback_loop() {
        let mut a = RenetClient::new(ConnectionConfig::default());
        let mut b = RenetClient::new(ConnectionConfig::default());
        for _ in 0..10_000 {
            a.send_message(DefaultChannel::Unreliable, Bytes::from_static(b"state"));
            deliver(a.get_packets_to_send(), &mut b);
            assert!(b.receive_message(DefaultChannel::Unreliable).is_some());
            deliver(b.get_packets_to_send(), &mut a);
            assert!(a.get_packets_to_send().is_empty());
            assert!(b.get_packets_to_send().is_empty());
            assert!(a.pending_acks.len() <= 1);
            assert!(b.pending_acks.len() <= 1);
            assert!(a.sent_packets.len() <= 1);
            assert!(b.sent_packets.len() <= 1);
        }
    }
