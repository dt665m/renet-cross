...
set -eu
cd "$(dirname "$0")/.."
cargo fmt -p renet-cross -p bevy-net-debug --check
...
cargo test -p bevy-net-debug --locked
cargo clippy -p bevy-net-debug --lib --tests --locked -- -D warnings
cargo clippy -p bevy-net-debug --target wasm32-unknown-unknown --locked -- -D warnings
git diff --check
